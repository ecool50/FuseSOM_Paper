...
Proximity metric: Euclidean, Manhattan
Correlation metric: Pearson, Cosine

```{r}
prox_metric <- c('euclidean')
corr_metric <- c('correlation')
```
...
res.corr.maldegem <- lapply(maldegem_subsets, run_pheno, markers = maldegem_markers, 
                              metric = 'correlation') %>% bind_rows()
res.prox.maldegem <- lapply(maldegem_subsets, run_pheno, markers = maldegem_markers, 
...
res.corr.rendeiro <- lapply(rendeiro_subsets, run_pheno, markers = rendeiro_markers, 
                              metric = 'correlation') %>% bind_rows()
res.prox.rendeiro <- lapply(rendeiro_subsets, run_pheno, markers = rendeiro_markers, 
                              metric = 'euclidean') %>% bind_rows()
...
res.corr.risom <- lapply(risom_subsets, run_pheno, markers = risom_markers, 
                              metric = 'correlation') %>% bind_rows()
res.prox.risom <- lapply(risom_subsets, run_pheno, markers = risom_markers, 
                              metric = 'euclidean') %>% bind_rows()
...
res.corr.damond <- lapply(damond_subsets, run_pheno, markers = damond_markers, 
                              metric = 'correlation') %>% bind_rows()
res.prox.damond <- lapply(damond_subsets, run_pheno, markers = damond_markers, 
                              metric = 'euclidean') %>% bind_rows()
...
res.corr.moldoveanu <- lapply(moldoveanu_subsets, run_pheno, markers = moldoveanu_markers, 
                              metric = 'correlation') %>% bind_rows()
res.prox.moldoveanu <- lapply(moldoveanu_subsets, run_pheno, markers = moldoveanu_markers, 
                              metric = 'euclidean') %>% bind_rows()
...
res.corr.hoch <- lapply(hoch_subsets, run_pheno, markers = hoch_markers, 
                          metric = 'correlation') %>% bind_rows()
res.prox.hoch <- lapply(hoch_subsets, run_pheno, markers = hoch_markers, 
                          metric = 'euclidean') %>% bind_rows()
...
res.corr.keren <- lapply(keren_subsets, run_pheno, markers = keren_markers, 
                          metric = 'correlation') %>% bind_rows()
res.prox.keren <- lapply(keren_subsets, run_pheno, markers = keren_markers, 
                          metric = 'euclidean') %>% bind_rows()
...
res.corr.mccaferry <- lapply(mccaferry_subsets, run_pheno, markers = mccaferry_markers, 
                               metric = 'correlation') %>% bind_rows()
res.prox.mccaferry <- lapply(mccaferry_subsets, run_pheno, markers = mccaferry_markers, 
                               metric = 'euclidean') %>% bind_rows()
...
res.corr.liu <- lapply(liu_subsets, run_pheno, markers = liu_markers, 
                         metric = 'correlation') %>% bind_rows()
res.prox.liu <- lapply(liu_subsets, run_pheno, markers = liu_markers, 
                         metric = 'euclidean') %>% bind_rows()
...
res.corr.schurch <- lapply(schurch_subsets, run_pheno, markers = schurch_markers, 
                             metric = 'correlation') %>% bind_rows()
res.prox.schurch <- lapply(schurch_subsets, run_pheno, markers = schurch_markers, 
                             metric = 'euclidean') %>% bind_rows()
...
res.corr.phillips <- lapply(phillips_subsets, run_pheno, markers = phillips_markers, 
                            metric = 'correlation') %>% bind_rows()
res.prox.phillips <- lapply(phillips_subsets, run_pheno, markers = phillips_markers, 
                            metric = 'euclidean') %>% bind_rows()
...
res.corr.lohoff <- lapply(lohoff_subsets, run_pheno, markers = lohoff_markers, 
                            metric = 'correlation') %>% bind_rows()
res.prox.lohoff <- lapply(lohoff_subsets, run_pheno, markers = lohoff_markers, 
                            metric = 'euclidean') %>% bind_rows()
...
res.corr.bortolomeazzi <- lapply(bortolomeazzi_subsets, run_pheno, markers = bortolomeazzi_markers, 
                            metric = 'correlation') %>% bind_rows()
res.prox.bortolomeazzi <- lapply(bortolomeazzi_subsets, run_pheno, markers = bortolomeazzi_markers, 
                            metric = 'euclidean') %>% bind_rows()
...
res.corr.brbic_int <- lapply(brbic_int_subsets, run_pheno, markers = brbic_int_markers, 
                            metric = 'correlation') %>% bind_rows()
res.prox.brbic_int <- lapply(brbic_int_subsets, run_pheno, markers = brbic_int_markers, 
                            metric = 'euclidean') %>% bind_rows()
...
res.corr.brbic_tons <- lapply(brbic_tons_subsets, run_pheno, markers = brbic_tons_markers, 
                            metric = 'correlation') %>% bind_rows()
res.prox.brbic_tons <- lapply(brbic_tons_subsets, run_pheno, markers = brbic_tons_markers, 
                            metric = 'euclidean') %>% bind_rows()
...
# write.csv(res.combined, '~/Documents/PhD/Spatial_Clustering/clustering/phenograph_analysis_results_all_20k.csv',
#           row.names = F)

```

```{r}
res.combined<- read.csv('/home/ewillie/Documents/PhD/Papers/FuseSOM_Paper/result_data/phenograph_analysis_results_all_20k.csv')
...
res.combined.melted$Metric <- factor(res.combined.melted$Metric, levels = c("correlation", "euclidean"))
```


# Generate bar plot
```{r}
p.bar.pheno <- ggplot(res.combined.melted, aes(x = Dataset, y = value, fill = Metric)) +
  ...
  ylab("Score") + theme(
    ...
    ) + scale_fill_jama() + ggtitle('PhenoGraph')
```
