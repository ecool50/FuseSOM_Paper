...
# 
# jackson_res <- runClustNum(jackson_dat, jackson_markers)
# 
# rm(jackson_dat)
# gc()
```
...
p <- ggplot(res.re, aes(x=Method, y=`Relative-Error`)) + geom_boxplot(fill='grey') +
  labs(title="",
  ...
  geom_hline(yintercept=0, linetype='dotted', col = 'black', linewidth = 3) 
...
p.metrics <- ggplot(res.metrics[res.metrics$Metric %like% 'ARI', ], aes(x=Method, y=Score)) + geom_boxplot(fill = 'grey') +
  labs(title="",
        x ="Cluster Estimation Method", y = "ARI") +
# facet_wrap(~Metric) + 
  theme_bw() +
  theme(
      axis.title.x = element_text(color="Black", size=25),
