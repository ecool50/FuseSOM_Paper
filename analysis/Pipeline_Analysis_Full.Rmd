...
# jackson_sce <- imcdatasets::JacksonFischer2020Data(data_type = "sce")
...
# # get the mean values
# res.jackson.full <- setDT(res.jackson.full)[,list(ARI=mean(ARI), NMI=mean(NMI), `FM-Index`=mean(`FM-Index`), FMeasure = mean(FMeasure)),
#                               by=c('Method')]
...
#           row.names = F)
res <- read.csv('~/Documents/PhD/Spatial_Clustering/clustering/FuseSOM_vs_FlowSOM_Vanilla.csv')
colnames(res)[[4]] <- 'FM-Index'

res.melted <- melt(res)
res.melted$Method <- factor(res.melted$Method, 
                           levels = c('FuseSOM', 'FlowSOM'))

p <- ggplot(res.melted, aes(x=Method, y=value)) + 
...
p.sig.ari <- ggwithinstats(
  ...
  centrality.plotting = F,
  subtitle  = 'p = 0.000004'
)  + theme(plot.subtitle = element_text(color = "black", size = 9, hjust = 0.5),
           axis.text.x = element_text(size = 11))

p.sig.nmi <- ggwithinstats(
  data            = res,
  x               = Method,
  y               = NMI,
  type            = "parametric",
...
```{r}
p.sig.final <- plot_grid(p.sig.ari, p.sig.nmi, p.sig.fm, p.sig.fmeasure)
```
